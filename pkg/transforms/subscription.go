...
func (s SubscriptionResource) BuildEdges(ns NodeStore) []Edge {
	...
	//subscribesTo edges
	if len(s.GetAnnotations()["app.open-cluster-management.io/deployables"]) > 0 {
		nodeInfo.EdgeType = "subscribesTo"
		deployableMap := make(map[string]struct{})
		for _, deployable := range strings.Split(s.GetAnnotations()["app.open-cluster-management.io/deployables"], ",") {

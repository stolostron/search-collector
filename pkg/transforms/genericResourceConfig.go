...
var defaultTransformConfig = map[string]ResourceConfig{
	...
	"VirtualMachine.kubevirt.io": {
		properties: []ExtractProperty{
			...
			{Name: "dataVolumeNames", JSONPath: `{.spec.template.spec.domain.volumes[*].dataVolume.name}`, DataType: DataTypeSlice},
			...
			{Name: "pvcClaimNames", JSONPath: `{.spec.template.spec.domain.volumes[*].persistentVolumeClaim.claimName}`, DataType: DataTypeSlice},

...
func generateSimpleEvent(informer GenericInformer, t *testing.T) {
	...
	_, err2 := informer.client.Resource(gvr).Namespace("ns-foo").Update(context.TODO(), newResource, v1.UpdateOptions{})

	// Delete resource. Generated DELETED event.
	err3 := informer.client.Resource(gvr).Namespace("ns-foo").Delete(context.TODO(), "name-bar2", v1.DeleteOptions{})
...
func Test_StoppedInformer_ValidateDeleteFunc(t *testing.T) {
	...
	// Verify that the informer.DeleteFunc was called with obj containing uid=id-999
	if uid != "id-999" {
